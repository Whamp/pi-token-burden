...
describe('loadSettings() / saveSettings()', () => {
  ...
  it('should discard malformed known fields while preserving other keys', () => {
    const { tmpDir, settingsPath } = makeTmpSettings();
    try {
      fs.writeFileSync(
        settingsPath,
        JSON.stringify({ skills: [1, 'valid'], packages: 'invalid', theme: 'dark' }),
      );

      expect(loadSettings(settingsPath)).toStrictEqual({ theme: 'dark' });
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });
