import { extractBaseLines } from './extractBaseLines.js';
...
describe('extractBaseLines()', () => {
  ...
  it('extracts guideline lines from Guidelines section', () => {
    ...
    expect(result.guidelineLines).toStrictEqual(['- Be concise', '- Show file paths clearly']);
  });
  ...
  it('handles tool lines with multi-word snippets', () => {
    ...
    expect(result.toolLines[0]).toBe('- web_search: Search the web using Perplexity AI or Gemini');
  });
  ...
  it('stops guideline lines at Pi documentation section', () => {
    ...
    expect(result.guidelineLines).toStrictEqual(['- Be concise', '- Show file paths']);
  });
});
