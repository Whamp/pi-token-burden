...
describe('attributeBasePrompt()', () => {
  const builtInToolLines = ['- read: Read file contents', '- bash: Execute bash commands'];
  ...
  it('attributes a tool line to a single extension', () => {
    ...
    const result = attributeBasePrompt(toolLines, builtInGuidelineLines, contributions, 100);
    ...
    expect(evidence?.contributors).toContain('/home/user/.pi/extensions/web-access/index.ts');
  });

  it('attributes built-in tool lines to the built-in bucket', () => {
    const result = attributeBasePrompt(builtInToolLines, builtInGuidelineLines, [], 100);

    const builtIn = result.buckets.find((b) => b.id === 'built-in');
    expect(builtIn).toBeDefined();
    expect(builtIn?.lineCount).toBe(builtInToolLines.length + builtInGuidelineLines.length);
  });
  ...
  it('attributes guideline lines from extensions', () => {
    ...
    const guidelineLines = [...builtInGuidelineLines, '- Always verify search results'];

    const result = attributeBasePrompt(builtInToolLines, guidelineLines, contributions, 100);

    const evidence = result.evidence.find((e) => e.line.includes('verify search'));
    expect(evidence?.bucket).toBe('extension');
  ...
  it('puts duplicate guidelines from multiple extensions into shared bucket', () => {
    ...
    const evidence = result.evidence.find((e) => e.line.includes('Use carefully'));
  ...
  it('puts unmatched lines into unattributed bucket', () => {
    const toolLines = [...builtInToolLines, '- mystery_tool: Does something unknown'];

    const result = attributeBasePrompt(toolLines, builtInGuidelineLines, [], 100);

    const evidence = result.evidence.find((e) => e.line.includes('mystery_tool'));
  ...
  it('reconciles: sum of bucket tokens equals total evidence tokens', () => {
    ...
    const result = attributeBasePrompt(toolLines, guidelineLines, contributions, 500);
  ...
  it('computes pctOfBase relative to baseTokens parameter', () => {
    const result = attributeBasePrompt(builtInToolLines, builtInGuidelineLines, [], 1000);
