...
import { extractContributions } from './extractContributions.js';
...
describe('extractContributions()', () => {
  it('extracts tool snippet and guidelines from a single extension', () => {
    ...
    const contribution = getRequiredItem(result, 0);
    expect(contribution.toolName).toBe('web_search');
    expect(contribution.snippet).toBe('Search the web');
    expect(contribution.guidelines).toStrictEqual(['Verify results before citing']);
    expect(contribution.extensionPath).toBe('/ext/web.ts');
  });
  ...
  it('extracts contributions from multiple extensions', () => {
    const ext1 = makeExtension('/ext/a.ts', [{ name: 'tool_a', snippet: 'Snippet A' }]);
    const ext2 = makeExtension('/ext/b.ts', [{ name: 'tool_b', snippet: 'Snippet B' }]);

    const result = extractContributions([ext1, ext2]);

    expect(result).toHaveLength(2);
    expect(getRequiredItem(result, 0).extensionPath).toBe('/ext/a.ts');
    expect(getRequiredItem(result, 1).extensionPath).toBe('/ext/b.ts');
  });
  ...
  it('skips tools without promptSnippet', () => {
    ...
    expect(result).toHaveLength(1);
    expect(getRequiredItem(result, 0).snippet).toBeUndefined();
  });
  ...
  it('returns empty guidelines array when tool has none', () => {
    ...
    expect(getRequiredItem(result, 0).guidelines).toStrictEqual([]);
  });
