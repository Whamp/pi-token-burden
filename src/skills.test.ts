...
describe('parseFrontmatter()', () => {
  ...
  it('should fall back to parent dir name when frontmatter has no name', () => {
    const content = ['---', 'description: Some skill', '---', '', '# Content'].join('\n');
...
describe('scanSkillDir()', () => {
  ...
  it('should discover SKILL.md files recursively', () => {
    ...
    try {
      ...
      const skill = getRequiredItem(skills, 0);
  ...
  it('should discover nested SKILL.md files', () => {
    ...
    try {
      ...
      expect(skills).toHaveLength(1);
      expect(getRequiredItem(skills, 0).name).toBe('nested-skill');
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });
  ...
  it('should discover root-level *.md files when includeRootFiles is true', () => {
    ...
    try {
      ...
      expect(skills).toHaveLength(1);
      expect(getRequiredItem(skills, 0).name).toBe('root-skill');
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });
  ...
  it('should honor ignore files when discovering skills', () => {
    ...
    try {
      ...
      expect(skills.map((skill) => skill.name)).toStrictEqual(['visible-skill']);
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });
...
describe('loadAllSkills()', () => {
  ...
  it('should apply configured package skill filters', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'skill-package-filter-'));
...
describe('property-based', () => {
  it('should roundtrip: written name matches discovered name', () => {
    fc.assert(
      fc.property(fc.stringMatching(/^[a-z][a-z0-9-]{0,10}[a-z0-9]$/), (name) => {
