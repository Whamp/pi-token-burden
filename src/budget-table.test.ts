import { buildTableItems } from "./budget-table.js";
...
describe("budget section table rows", () => {
  it("marks Skills Budget Sections as drillable through the shared section predicate", () => {
    ...
    const skillsItem = buildTableItems(parsed).find(
      (item) => item.label === "Skills (0)"
    );
  ...
  it("preserves table item sorting, percentages, content, and child rows", () => {
    ...
    `);
    expect(items.find((item) => item.label === "Base prompt")?.content).toBe(
      "base instructions"
    );
    expect(
      items.find((item) => item.label === "AGENTS.md files")?.children?.[0]
        ?.content
    ).toBe("user agents");
